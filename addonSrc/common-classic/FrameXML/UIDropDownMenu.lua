...
local VIDEO_QUALITY_LABEL6 = VIDEO_QUALITY_LABEL6

local CloseDropDownMenus
local ColorPicker_GetPreviousValues
local HideDropDownMenu
local OPEN_DROPDOWNMENUS
local OpenColorPicker
local ToggleDropDownMenu
local UIDropDownMenu_AddButton
local UIDropDownMenu_AddSeparator
local UIDROPDOWNMENU_BORDER_HEIGHT
local UIDROPDOWNMENU_BUTTON_HEIGHT
local UIDropDownMenu_CheckAddCustomFrame
local UIDROPDOWNMENU_CLASSIC_CHECK_PADDING
local UIDropDownMenu_ClearAll
local UIDropDownMenu_CreateFrames
local UIDropDownMenu_CreateInfo
local UIDROPDOWNMENU_DEFAULT_TEXT_HEIGHT
local UIDropDownMenu_DisableButton
local UIDropDownMenu_DisableDropDown
local UIDropDownMenu_EnableButton
local UIDropDownMenu_EnableDropDown
local UIDropDownMenu_GetButtonWidth
local UIDropDownMenu_GetCurrentDropDown
local UIDropDownMenu_GetMaxButtonWidth
local UIDropDownMenu_GetSelectedID
local UIDropDownMenu_GetSelectedName
local UIDropDownMenu_GetSelectedValue
local UIDropDownMenu_GetText
local UIDropDownMenu_GetValue
local UIDropDownMenu_IsEnabled
local UIDROPDOWNMENU_INIT_MENU
local UIDropDownMenu_Initialize
local UIDropDownMenu_InitializeHelper
local UIDropDownMenu_JustifyText
local UIDROPDOWNMENU_MAXBUTTONS
local UIDROPDOWNMENU_MAXLEVELS
local UIDROPDOWNMENU_MENU_LEVEL
local UIDROPDOWNMENU_MENU_VALUE
local UIDROPDOWNMENU_MINBUTTONS
local UIDropDownMenu_OnHide
local UIDropDownMenu_OnUpdate
local UIDROPDOWNMENU_OPEN_MENU
local UIDropDownMenu_Refresh
local UIDropDownMenu_RefreshAll
local UIDropDownMenu_RefreshDropDownSize
local UIDropDownMenu_RegisterCustomFrame
local UIDropDownMenu_SetAnchor
local UIDropDownMenu_SetButtonClickable
local UIDropDownMenu_SetButtonNotClickable
local UIDropDownMenu_SetButtonText
local UIDropDownMenu_SetButtonWidth
local UIDropDownMenu_SetDisplayMode
local UIDropDownMenu_SetIconImage
local UIDropDownMenu_SetInitializeFunction
local UIDropDownMenu_SetSelectedID
local UIDropDownMenu_SetSelectedName
local UIDropDownMenu_SetSelectedValue
local UIDropDownMenu_SetText
local UIDropDownMenu_SetWidth
local UIDROPDOWNMENU_SHOW_TIME
local UIDropDownMenu_StopCounting
local UIDropDownMenuButton_GetChecked
local UIDropDownMenuButton_GetName
local UIDropDownMenuButton_OnClick
local UIDropDownMenuButton_OnEnter
local UIDropDownMenuButton_OnLeave
local UIDropDownMenuButton_OpenColorPicker
local UIDropDownMenu_StartCounting
local UIDropDownMenuButtonInvisibleButton_OnEnter
local UIDropDownMenuButtonInvisibleButton_OnLeave
local UIDropDownMenuDelegate_OnAttributeChanged
...
function UIDropDownMenu_Initialize(frame, initFunction, displayMode, level, menuList)
	...
	local dropDownList = _G["TomCats_DropDownList"..level];
...
function UIDropDownMenuButton_OnEnter(self)
	if ( self.hasArrow ) then
		...
		local listFrame = _G["TomCats_DropDownList"..level];
...
function UIDropDownMenu_CreateFrames(level, index)
	while ( level > UIDROPDOWNMENU_MAXLEVELS ) do
		...
		local newList = CreateFrame("Button", "TomCats_DropDownList"..UIDROPDOWNMENU_MAXLEVELS, nil, "TomCats_UIDropDownListTemplate");
		...
		for i=UIDROPDOWNMENU_MINBUTTONS+1, UIDROPDOWNMENU_MAXBUTTONS do
			local newButton = CreateFrame("Button", "TomCats_DropDownList"..UIDROPDOWNMENU_MAXLEVELS.."Button"..i, newList, "TomCats_UIDropDownMenuButtonTemplate");
			newButton:SetID(i);
		end
	end
	...
	while ( index > UIDROPDOWNMENU_MAXBUTTONS ) do
		...
		for i=1, UIDROPDOWNMENU_MAXLEVELS do
			local newButton = CreateFrame("Button", "TomCats_DropDownList"..i.."Button"..UIDROPDOWNMENU_MAXBUTTONS, _G["TomCats_DropDownList"..i], "TomCats_UIDropDownMenuButtonTemplate");
...
function UIDropDownMenu_AddButton(info, level)
	...
	listFrame = listFrame or _G["TomCats_DropDownList"..level];
	...
	if ( info.hasColorSwatch ) then
		_G["TomCats_DropDownList"..level.."Button"..index.."ColorSwatch".."NormalTexture"]:SetVertexColor(info.r, info.g, info.b);
		button.r = info.r;
		button.g = info.g;
		button.b = info.b;
		colorSwatch:Show();
...
function UIDropDownMenu_Refresh(frame, useValue, dropdownLevel)
	...
	local listFrame = _G["TomCats_DropDownList"..dropdownLevel];
	listFrame.numButtons = listFrame.numButtons or 0;
	-- Just redraws the existing menu
	for i=1, UIDROPDOWNMENU_MAXBUTTONS do
		local button = _G["TomCats_DropDownList"..dropdownLevel.."Button"..i];
		local checked = nil;
		...
		if not button.notCheckable and button:IsShown() then
			...
			local checkImage = _G["TomCats_DropDownList"..dropdownLevel.."Button"..i.."Check"];
			local uncheckImage = _G["TomCats_DropDownList"..dropdownLevel.."Button"..i.."UnCheck"];
			if ( checked ) then
	...
	if (not frame.noResize) then
		...
		UIDropDownMenu_RefreshDropDownSize(_G["TomCats_DropDownList"..dropdownLevel]);
	end
end
...
function UIDropDownMenu_GetSelectedID(frame)
	if ( frame.selectedID ) then
		...
		local button;
		for i=1, UIDROPDOWNMENU_MAXBUTTONS do
			button = _G["TomCats_DropDownList"..UIDROPDOWNMENU_MENU_LEVEL.."Button"..i];
			-- See if checked or not
			if ( UIDropDownMenu_GetSelectedName(frame) ) then
				if ( button:GetText() == UIDropDownMenu_GetSelectedName(frame) ) then
					return i;
...
function ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button, autoHideDelay)
	...
	local listFrame = _G["TomCats_DropDownList"..level];
	local listFrameName = "TomCats_DropDownList"..level;
	local tempFrame;
	...
	if ( listFrame:IsShown() and (UIDROPDOWNMENU_OPEN_MENU == tempFrame) ) then
		...
		if ( level == 1 ) then
			...
			if ( strsub(button:GetParent():GetName(), 0,20) == "TomCats_DropDownList" and strlen(button:GetParent():GetName()) == 13 ) then
				anchorFrame = button;
			else
		...
		if ( level == 1 ) then
			...
			listFrame:ClearAllPoints();
			listFrame.parentLevel = tonumber(strmatch(anchorFrame:GetName(), "TomCats_DropDownList(%d+)"));
...
function CloseDropDownMenus(level)
	...
	for i=level, UIDROPDOWNMENU_MAXLEVELS do
		_G["TomCats_DropDownList"..i]:Hide();
	end
end
...
function UIDropDownMenu_ClearAll(frame)
	...
	for i=1, UIDROPDOWNMENU_MAXBUTTONS do
		...
		checkImage = _G["TomCats_DropDownList"..UIDROPDOWNMENU_MENU_LEVEL.."Button"..i.."Check"];
		checkImage:Hide();
		uncheckImage = _G["TomCats_DropDownList"..UIDROPDOWNMENU_MENU_LEVEL.."Button"..i.."UnCheck"];
		uncheckImage:Hide();
	end
end
...
function UIDropDownMenu_DisableButton(level, id)
	_G["TomCats_DropDownList"..level.."Button"..id]:Disable();
end

function UIDropDownMenu_EnableButton(level, id)
	_G["TomCats_DropDownList"..level.."Button"..id]:Enable();
end

function UIDropDownMenu_SetButtonText(level, id, text, colorCode)
	local button = _G["TomCats_DropDownList"..level.."Button"..id];
	if ( colorCode) then
...
function UIDropDownMenu_SetButtonNotClickable(level, id)
	_G["TomCats_DropDownList"..level.."Button"..id]:SetDisabledFontObject(GameFontHighlightSmallLeft);
end

function UIDropDownMenu_SetButtonClickable(level, id)
	_G["TomCats_DropDownList"..level.."Button"..id]:SetDisabledFontObject(GameFontDisableSmallLeft);
end
...
function UIDropDownMenu_GetValue(id)
	...
	local button = _G["TomCats_DropDownList1Button"..id];
	if ( button ) then
		return _G["TomCats_DropDownList1Button"..id].value;
	else
		return nil;
	end
end
...
TomCats_ToggleDropDownMenu = ToggleDropDownMenu
TomCats_UIDropDownMenu_AddButton = UIDropDownMenu_AddButton
TomCats_UIDropDownMenu_CreateInfo = UIDropDownMenu_CreateInfo
TomCats_UIDropDownMenu_Initialize = UIDropDownMenu_Initialize
TomCats_CloseDropDownMenus = CloseDropDownMenus

TomCats_UIDropDownMenu_AddSeparator = UIDropDownMenu_AddSeparator
TomCats_UIDropDownMenu_SetDisplayMode = UIDropDownMenu_SetDisplayMode
TomCats_UIDropDownMenu_SetInitializeFunction = UIDropDownMenu_SetInitializeFunction
TomCats_UIDropDownMenu_SetWidth = UIDropDownMenu_SetWidth
TomCats_UIDropDownMenu_SetSelectedValue = UIDropDownMenu_SetSelectedValue
TomCats_UIDropDownMenu_StopCounting = UIDropDownMenu_StopCounting
TomCats_UIDropDownMenu_StartCounting = UIDropDownMenu_StartCounting

TomCats_UIDropDownMenuButtonInvisibleButton_OnEnter = UIDropDownMenuButtonInvisibleButton_OnEnter
TomCats_UIDropDownMenuButtonInvisibleButton_OnLeave = UIDropDownMenuButtonInvisibleButton_OnLeave
TomCats_UIDropDownMenuButton_OnClick = UIDropDownMenuButton_OnClick
TomCats_UIDropDownMenuButton_OnEnter = UIDropDownMenuButton_OnEnter
TomCats_UIDropDownMenuButton_OnLeave = UIDropDownMenuButton_OnLeave
TomCats_UIDropDownMenuButton_OpenColorPicker = UIDropDownMenuButton_OpenColorPicker

TomCats_UIDropDownMenu_OnUpdate = UIDropDownMenu_OnUpdate
TomCats_UIDROPDOWNMENU_MAXBUTTONS = UIDROPDOWNMENU_MAXBUTTONS
TomCats_UIDropDownMenu_OnHide = UIDropDownMenu_OnHide
