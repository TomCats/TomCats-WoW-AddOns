...
local ABANDON_QUEST = ABANDON_QUEST
local AUTO_QUEST_WATCH = AUTO_QUEST_WATCH
local C_QuestLog = TomCats_C_QuestLog
local C_Map = TomCats_C_Map
local C_PlayerInfo = C_PlayerInfo
local C_TaskQuest = C_TaskQuest
local C_SuperTrack = TomCats_C_SuperTrack
local ChatEdit_TryInsertQuestLinkForQuestID = ChatEdit_TryInsertQuestLinkForQuestID
local CLICK_QUEST_DETAILS = CLICK_QUEST_DETAILS
local CloseDropDownMenus = TomCats_CloseDropDownMenus
local CollapseQuestHeader = CollapseQuestHeader
local Constants = Constants
local CreateFrame = CreateFrame
local CreateFramePool_Orig = CreateFramePool
local DAILY = DAILY
local DISABLED_FONT_COLOR = DISABLED_FONT_COLOR
local ENABLE_COLORBLIND_MODE = ENABLE_COLORBLIND_MODE
local Enum = Enum
...
local format = format
local FramePool_HideAndClearAnchors = FramePool_HideAndClearAnchors
...
local GameTooltip_CheckAddQuestTimeToTooltip = GameTooltip_CheckAddQuestTimeToTooltip
...
local GetQuestLogPortraitGiver = GetQuestLogPortraitGiver
local GetQuestLogQuestText = GetQuestLogQuestText
local GetQuestSessionHelpText = GetQuestSessionHelpText
...
local IsTutorialFlagged = IsTutorialFlagged
local Item = Item
local LE_QUEST_FACTION_HORDE = LE_QUEST_FACTION_HORDE
local MapQuestInfoRewardsFrame = MapQuestInfoRewardsFrame
local max = max
local min = min
local MIN_STORY_TOOLTIP_WIDTH = MIN_STORY_TOOLTIP_WIDTH
local NORMAL_FONT_COLOR = NORMAL_FONT_COLOR
local ObjectiveTracker_UpdatePOIs = ObjectiveTracker_UpdatePOIs
local OpenQuestLog = OpenQuestLog
local PARTY_QUEST_STATUS_ON = PARTY_QUEST_STATUS_ON
local PlaySound = PlaySound
local QUEST_DASH = QUEST_DASH
local QUEST_SESSION_ON_HOLD_TOOLTIP_TITLE = QUEST_SESSION_ON_HOLD_TOOLTIP_TITLE
local QUEST_SESSION_QUEST_TOOLTIP_IS_REPLAY = QUEST_SESSION_QUEST_TOOLTIP_IS_REPLAY
local QUEST_SESSION_TOOLTIP_START_SESSION_NOT_IN_COMBAT = QUEST_SESSION_TOOLTIP_START_SESSION_NOT_IN_COMBAT
local QUEST_SESSION_WORLD_MAP_TUTORIAL_TEXT = QUEST_SESSION_WORLD_MAP_TUTORIAL_TEXT
local QUEST_STORY_STATUS = QUEST_STORY_STATUS
local QUEST_TAG_TCOORDS = QUEST_TAG_TCOORDS
local QUEST_TEMPLATE_LOG = QUEST_TEMPLATE_LOG
local QUEST_TEMPLATE_MAP_DETAILS = QUEST_TEMPLATE_MAP_DETAILS
local QUEST_TEMPLATE_MAP_REWARDS = QUEST_TEMPLATE_MAP_REWARDS
local QUEST_WATCH_QUEST_READY = QUEST_WATCH_QUEST_READY
local QuestCache = QuestCache
local QuestDifficultyHighlightColors = QuestDifficultyHighlightColors
local QuestFrame_HideQuestPortrait = QuestFrame_HideQuestPortrait
local QuestFrame_ShowQuestPortrait = QuestFrame_ShowQuestPortrait
local QuestInfo_Display = QuestInfo_Display
local QuestInfo_AdjustTitleWidth = QuestInfo_AdjustTitleWidth
local QuestLogPushQuest = QuestLogPushQuest
local QuestLogShouldShowPortrait = QuestLogShouldShowPortrait
local QuestMapDetailsScrollFrame = TomCats_QuestMapDetailsScrollFrame
local QuestModelScene = QuestModelScene

--todo: implement QuestPOI_* functions from QuestPOI.lua
local GetQuestPOIs = nop
local QuestPOI_GetButton = nop
local QuestPOI_Initialize = nop
local QuestPOI_ResetUsage = nop
local QuestPOI_HideUnusedButtons = nop
local QuestPOI_SelectButtonByQuestID = nop

local QuestSessionManager = QuestSessionManager
local QuestUtils_AddQuestTagLineToTooltip = TomCats_QuestUtils_AddQuestTagLineToTooltip
local QuestUtils_GetReplayQuestDecoration = TomCats_QuestUtils_GetReplayQuestDecoration
local QuestUtils_IsQuestWatched = TomCats_QuestUtils_IsQuestWatched
local QuestUtils_DecorateQuestText = TomCats_QuestUtils_DecorateQuestText
local QuestUtils_GetDisabledQuestDecoration = TomCats_QuestUtils_GetDisabledQuestDecoration
local QuestUtils_GetNumPartyMembersOnQuest = TomCats_QuestUtils_GetNumPartyMembersOnQuest
local QuestUtils_GetQuestName = TomCats_QuestUtils_GetQuestName
local RED_FONT_COLOR = RED_FONT_COLOR
local ScrollFrame_OnLoad = ScrollFrame_OnLoad
local sessionCommandToButtonAtlases = sessionCommandToButtonAtlases
local sessionCommandToCommandName = sessionCommandToCommandName
local sessionCommandToTooltipBody = sessionCommandToTooltipBody
local sessionCommandToTooltipTitle = sessionCommandToTooltipTitle
local SetupObjectiveTextColor = SetupObjectiveTextColor
local SHARE_QUEST = SHARE_QUEST
local ShowUIPanel = ShowUIPanel
local SortQuests = SortQuests
local SortQuestSortTypes = SortQuestSortTypes
local SOUNDKIT = SOUNDKIT
local StaticPopup_Hide = StaticPopup_Hide
local StaticPopup_Show = StaticPopup_Show
local STORY_CHAPTERS = STORY_CHAPTERS
local ToggleDropDownMenu = TomCats_ToggleDropDownMenu
local tooltipButton = tooltipButton
local TRACK_QUEST = TRACK_QUEST
local TRACK_QUEST_ABBREV = TRACK_QUEST_ABBREV
local TriggerTutorial = TriggerTutorial
local TUTORIAL_QUEST_ACCEPTED = TUTORIAL_QUEST_ACCEPTED
local TUTORIAL_QUEST_TO_WATCH = TUTORIAL_QUEST_TO_WATCH
...
local UIDROPDOWNMENU_MENU_LEVEL = TomCats_UIDROPDOWNMENU_MENU_LEVEL
...
local UNTRACK_QUEST = UNTRACK_QUEST
...
local QuestDetailsFrame_OnHide
local QuestDetailsFrame_OnShow
local QuestLogHeaderCodeMixin
local QuestLogHeaderMixin
local QuestLogMixin
local QuestLogPopupDetailFrame
local QuestLogPopupDetailFrame_OnHide
local QuestLogPopupDetailFrame_OnLoad
local QuestLogPopupDetailFrame_Show
local QuestLogPopupDetailFrame_Update
local QuestLogQuests_Update
local QuestMapFrame
local QuestMapFrame_AdjustPathButtons
local QuestMapFrame_CheckAutoSupertrackOnShowDetails
local QuestMapFrame_CheckQuestCriteria
local QuestMapFrame_CloseQuestDetails
local QuestMapFrame_OnEvent
local QuestMapFrame_OnHide
local QuestMapFrame_OnLoad
local QuestMapFrame_OnShow
local QuestMapFrame_OpenToQuestDetails
local QuestMapFrame_ReturnFromQuestDetails
local QuestMapFrame_ResetFilters
local QuestMapFrame_ShowQuestDetails
local QuestMapFrame_ToggleShowDestination
local QuestMapFrame_UpdateAll
local QuestMapFrame_UpdateAllQuestCriteria
local QuestMapFrame_UpdateQuestDetailsButtons
local QuestMapFrame_UpdateQuestSessionState
local QuestMapFrame_UpdateSuperTrackedQuest
local QuestMapLog_HideStoryTooltip
local QuestMapLog_ShowStoryTooltip
local QuestMapLogTitleButton_OnClick
local QuestMapLogTitleButton_OnEnter
local QuestMapLogTitleButton_OnLeave
local QuestMapLogTitleButton_OnMouseDown
local QuestMapLogTitleButton_OnMouseUp
local QuestMapQuestOptions_AbandonQuest
local QuestMapQuestOptions_ShareQuest
local QuestMapQuestOptions_TrackQuest
local QuestMapQuestOptionsDropDown
local QuestMapQuestOptionsDropDown_Initialize
local QuestScrollFrame
local QuestSessionManagementExecute_OnClick
local QuestSessionManagement_OnEnter
local QuestSessionManagement_OnLeave
local QuestSessionManagementMixin
local QuestsFrame_OnLoad

local function CreateFramePool(arg1, arg2, arg3, arg4)
	return CreateFramePool_Orig(arg1, arg2, "TomCats_" .. arg3, arg4)
end

--todo: implement function
local function QuestMapUpdateAllQuests()
	return 0
end

--todo: implement function
local function QuestPOIUpdateIcons()
end
...
function QuestMapFrame_OnLoad(self)
	...
	--todo: work-around the unavailable events
...
TomCats_QuestDetailsFrame_OnHide = QuestDetailsFrame_OnHide
TomCats_QuestDetailsFrame_OnShow = QuestDetailsFrame_OnShow
TomCats_QuestLogHeaderCodeMixin = QuestLogHeaderCodeMixin
TomCats_QuestLogHeaderMixin = QuestLogHeaderMixin
TomCats_QuestLogMixin = QuestLogMixin
TomCats_QuestLogPopupDetailFrame_OnHide = QuestLogPopupDetailFrame_OnHide
TomCats_QuestLogPopupDetailFrame_OnLoad = QuestLogPopupDetailFrame_OnLoad
TomCats_QuestLogPopupDetailFrame_Show = QuestLogPopupDetailFrame_Show
TomCats_QuestLogPopupDetailFrame_Update = QuestLogPopupDetailFrame_Update
TomCats_QuestMapFrame_OnEvent = QuestMapFrame_OnEvent
TomCats_QuestMapFrame_OnLoad = QuestMapFrame_OnLoad
TomCats_QuestMapFrame_OnHide = QuestMapFrame_OnHide
TomCats_QuestMapFrame_OnShow = QuestMapFrame_OnShow
TomCats_QuestMapFrame_ShowQuestDetails = QuestMapFrame_ShowQuestDetails
TomCats_QuestMapFrame_ToggleShowDestination = QuestMapFrame_ToggleShowDestination
TomCats_QuestMapLog_HideStoryTooltip = QuestMapLog_HideStoryTooltip
TomCats_QuestMapLog_ShowStoryTooltip = QuestMapLog_ShowStoryTooltip
TomCats_QuestMapLogTitleButton_OnClick = QuestMapLogTitleButton_OnClick
TomCats_QuestMapLogTitleButton_OnEnter = QuestMapLogTitleButton_OnEnter
TomCats_QuestMapLogTitleButton_OnLeave = QuestMapLogTitleButton_OnLeave
TomCats_QuestMapLogTitleButton_OnMouseDown = QuestMapLogTitleButton_OnMouseDown
TomCats_QuestMapLogTitleButton_OnMouseUp = QuestMapLogTitleButton_OnMouseUp
TomCats_QuestSessionManagementExecute_OnClick = QuestSessionManagementExecute_OnClick
TomCats_QuestSessionManagement_OnEnter = QuestSessionManagement_OnEnter
TomCats_QuestSessionManagement_OnLeave = QuestSessionManagement_OnLeave
TomCats_QuestSessionManagementMixin = QuestSessionManagementMixin

function TomCats_QuestLogPopupDetailFrame_OnLoad(self)
	QuestLogPopupDetailFrame = self
	QuestLogPopupDetailFrame_OnLoad(self)
end

function TomCats_QuestsFrame_OnLoad(self)
	QuestScrollFrame = self
	QuestMapFrame = QuestScrollFrame:GetParent()
	QuestsFrame_OnLoad(self)
end

function TomCats_QuestMapQuestOptionsDropDown_OnLoad(self)
	QuestMapQuestOptionsDropDown = self
end

local function ADDON_LOADED(_, arg1)
	if (addonName == arg1) then
		addon.UnregisterEvent("ADDON_LOADED", ADDON_LOADED)
		QuestLogPopupDetailFrame = TomCats_QuestLogPopupDetailFrame
	end
end
