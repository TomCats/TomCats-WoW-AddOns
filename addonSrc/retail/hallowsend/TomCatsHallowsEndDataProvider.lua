...
TomCatsHallowsEndDataProviderMixin = CreateFromMixins(MapCanvasDataProviderMixin)
function TomCatsHallowsEndDataProviderMixin.UpdatePreferences()
...
TomCatsHallowsEndDataProviderMixin.activePins = {}
TomCatsHallowsEndDataProviderMixin.activeEntrancePins = {}
TomCatsHallowsEndDataProviderMixin.activePathways = {}
TomCatsHallowsEndDataProviderMixin.activePOIs = {}

function TomCatsHallowsEndDataProviderMixin:AcquireLine()
    local line = self.linePool:Acquire()
    allPins[line] = line
    return line
end

function TomCatsHallowsEndDataProviderMixin:ReleaseLine(line)
    allPins[line] = nil
    self.linePool:Release(line)
    line:Hide()
end

function TomCatsHallowsEndDataProviderMixin:QUEST_TURNED_IN(_, questID)
...
function TomCatsHallowsEndDataProviderMixin:QUEST_LOG_UPDATE()
...
function TomCatsHallowsEndDataProviderMixin:OnAdded(owningMap)
    MapCanvasDataProviderMixin.OnAdded(self, owningMap)
    if not self.linePool then
        self.linePool = CreateFramePool("FRAME", self:GetMap():GetCanvas(), "TomCatsHallowsEndPathwayLineTemplate");
    end
...
function TomCatsHallowsEndDataProviderMixin:RefreshAllData(fromOnShow)
            ...
            if (canShowEntrance(entrance)) then
                ...
                if (P.showCompleted or (not allCompleted)) then
                    ...
                    if (location) then
                        ...
                        self.activeEntrancePins[entrance["Entrance ID"]] = self:GetMap():AcquirePin("TomCatsHallowsEndPinTemplate", pinInfo)
                    end
                end
            end
        end
            ...
            if (C_Map.GetMapArtID(phasedZone["UIMap ID"]) ~= phasedZone["Timewalking Map Art ID"]) then
                ...
                if (displayPOI) then
                    ...
                    if (poiInfo.position) then
                        ...
                        self.activePOIs[poiInfo.areaPoiID] = self:GetMap():AcquirePin("TomCatsHallowsEndAreaPOIPinTemplate", poiInfo)
                    end
                end
            end
        end
...
function TomCatsHallowsEndPinMixin:OnAcquired(pinInfo)
    self.completed = nil;
...
function TomCatsHallowsEndPinMixin:OnCanvasScaleChanged()
...
function TomCatsHallowsEndPinMixin:HideItemTooltip()
    WorldMapTooltip:Hide()
end
function TomCatsHallowsEndPinMixin:SetIconState(state)
...
function TomCatsHallowsEndPinMixin:OnMouseEnter()
    self:SetIconState("Highlighted")
    self:ShowTooltip()
end
function TomCatsHallowsEndPinMixin:OnMouseLeave()
...
function TomCatsHallowsEndPinMixin:OnMouseDown()
--    setIconState(self.iconPushed)
--    self:HideItemTooltip()
end
function TomCatsHallowsEndPinMixin:OnMouseUp(button)
    if (TomTom) then
...
function TomCatsHallowsEndPinMixin:GetIconStates()
    return { self.iconDefault, self.iconHighlighted, self.iconPushed, self.iconCompleted }
end

local function OnUpdate()
    local enabledVar = TomCats_Account and TomCats_Account.hallowsend and TomCats_Account.hallowsend.iconsEnabled
